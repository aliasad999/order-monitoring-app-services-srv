...
describe('Start Server and check various ODATA  calls', function () {
  ...
  it(` All Fields from the app  ==> for the month of ${currentMonthName}`, async function () {
    ...
    this.timeout(timeout);
    try {
      const response = await axios.get(`${baseUrl}/Results?$skip=0&$top=981&$filter=(SO_EDATU_REQUESTED ge ${firstDay} and SO_EDATU_REQUESTED le ${lastDay})&$select=BL_MANDT_INV_FIRST,BL_MANDT_INV_LAST,TM_MANDT,DL_MANDT,SO_MANDT,SO_VBELN,SO_POSNR,SO_AUART,SO_ERDAT_ORDER,SO_ERDAT_ITEM,SO_WERKS,SO_VTWEG,SO_MATNR,SO_MAKTX,SO_KDMAT,SO_AG_PARTNER,SO_AG_PARTNER_NAME,SO_WE_PARTNER,SO_WE_PARTNER_NAME,SO_LAND1,SO_LANDX,SO_ORT01,SO_VKORG,SO_VKORG_NAME1,SO_KNREF_HEAD,SO_VBUND,SO_EDATU_REQUESTED,SO_KWMENG,SO_VRKME,SO_EDATU_CONFIRMED,SO_KBMENG,SO_F_LDDAT,SO_UNCONFIRMED_QTY,SO_REQ_TEXT,SO_FAKSP,SO_FAKSP_VTEXT,SO_F_LGORT,SO_SUPPLY_SITUATION,SO_SUPPLY_SITUATION_DESCR,SO_HTEXT,SO_PSTYV,SO_PSTYV_VTEXT,SO_DISPO,SO_KOSCH,SO_VKBUR,SO_VKBUR_BEZEI,SO_CO_PARTNER,SO_CO_PARTNER_NAME,SO_NY_PARTNER,SO_NY_PARTNER_NAME,SO_AS_PARTNER,SO_AS_PARTNER_NAME,SO_VE_PARTNER,SO_VE_PARTNER_NAME,SO_AM_PARTNER,SO_AM_PARTNER_NAME,DL_WADAT_IST,SO_F_ZZ0S2MATUG,SO_F_AS_PARTNER,SO_F_AS_PARTNER_NAME,TM_DATEN,TM_DATBG,DL_ZZ0S2BLNR,SO_F_DGLTP,DL_CHARG,BL_VBELN_INV_LAST,BL_VBELN_INV_FIRST,TM_AR_DATE,DL_TRAID,SO_ZZ0S2REVG2,DL_VBELN,DL_POSNR,DL_LFIMG,DL_VRKME,DL_LFART,DL_LFART_VTEXT,TM_DPTEN,TM_DPTBG,SO_F_VBELN,SO_I_VBELN,TM_TDLNR,TM_TDLNR_NAME1,SO_GUSCON_LEVEL,SO_INCO1,SO_INCO2,SO_F_PSMNG,SO_F_AMEIN,SO_BASF_LOFCR,LAST_NOTE,SO_LEVEL_TYPE,SO_NETWR,SO_N_VBELN,BL_XBLNR,SO_ISCOMPLETED,TM_STTRG,TM_STTRG_DDTEXT,SO_ZTERM,DL_PEND_DEL_QUAN,DL_LFDAT,DL_WADAT,SO_F_WERKS,SO_BSARK,SO_BSARK_VTEXT,SO_F_VKORG,SO_F_VKORG_VTEXT,SO_KBETR,SO_PRSDT,SO_F_AUFNR,SO_BSTKD,SO_ROUTE,SO_VKGRP,SO_VKGRP_BEZEI,SO_ZZDKPPRODB,TM_TKNUM,TM_SHIPMENT_ALERT,TM_SHIPMENT_CURRENT_STATUS,SO_F_VSBED,SO_F_VSBED_VTEXT,TM_VSART,TM_VSART_BEZEI,SO_TRAGR,SO_TRAGR_VTEXT,SO_F_TDDAT,TM_EXTI1,DL_HSDAT,DL_VFDAT,SO_VBTYP,TM_TRACKING_ID_ELEM,TM_TRACKING_ID_COMP,SO_WAERK,SO_WAERS`)
      assert.equal(200, response.status)
    } catch (error) {
      assert.strictEqual(error.code, 'ECONNABORTED');
      assert.strictEqual(error.message, `timeout of ${timeout}ms exceeded`);
  ...
  it(` delivery + shipment + billing first and final for ==> ${currentMonthName}`, async function () {
    ...
    this.timeout(timeout);
    try {
      const response = await axios.get(`${baseUrl}/Results?$skip=0&$top=981&$filter=(SO_EDATU_REQUESTED ge ${firstDay} and SO_EDATU_REQUESTED le ${lastDay})&$select=TM_MANDT,DL_MANDT,BL_MANDT_INV_FIRST,BL_MANDT_INV_LAST,DL_CHARG,DL_VBELN,DL_POSNR,TM_TKNUM,TM_SHIPMENT_CURRENT_STATUS,TM_SHIPMENT_ALERT,BL_VBELN_INV_FIRST,BL_VBELN_INV_LAST,DL_LFIMG,DL_LFART,DL_LFDAT,DL_TRAID,DL_ZZ0S2BLNR,DL_PEND_DEL_QUAN,DL_WADAT,DL_WADAT_IST,TM_VSART,TM_EXTI1,TM_TDLNR,TM_DPTBG,TM_DATBG,TM_DPTEN,TM_DATEN,TM_AR_DATE,TM_STTRG,BL_XBLNR`)
      assert.equal(200, response.status)
    } catch (error) {
      assert.strictEqual(error.code, 'ECONNABORTED');
      assert.strictEqual(error.message, `timeout of ${timeout}ms exceeded`);
  ...
  it(` All Fields from the app for ==> ${currentQuarter} Quarter`, async function () {
    ...
    this.timeout(timeout);
    try {
      const response = await axios.get(`${baseUrl}/Results?$skip=0&$top=981&$filter=(SO_EDATU_REQUESTED ge ${firstDayQuarter} and SO_EDATU_REQUESTED le ${lastDayQuarter})&$select=BL_MANDT_INV_FIRST,BL_MANDT_INV_LAST,TM_MANDT,DL_MANDT,SO_MANDT,SO_VBELN,SO_POSNR,SO_AUART,SO_ERDAT_ORDER,SO_ERDAT_ITEM,SO_WERKS,SO_VTWEG,SO_MATNR,SO_MAKTX,SO_KDMAT,SO_AG_PARTNER,SO_AG_PARTNER_NAME,SO_WE_PARTNER,SO_WE_PARTNER_NAME,SO_LAND1,SO_LANDX,SO_ORT01,SO_VKORG,SO_VKORG_NAME1,SO_KNREF_HEAD,SO_VBUND,SO_EDATU_REQUESTED,SO_KWMENG,SO_VRKME,SO_EDATU_CONFIRMED,SO_KBMENG,SO_F_LDDAT,SO_UNCONFIRMED_QTY,SO_REQ_TEXT,SO_FAKSP,SO_FAKSP_VTEXT,SO_F_LGORT,SO_SUPPLY_SITUATION,SO_SUPPLY_SITUATION_DESCR,SO_HTEXT,SO_PSTYV,SO_PSTYV_VTEXT,SO_DISPO,SO_KOSCH,SO_VKBUR,SO_VKBUR_BEZEI,SO_CO_PARTNER,SO_CO_PARTNER_NAME,SO_NY_PARTNER,SO_NY_PARTNER_NAME,SO_AS_PARTNER,SO_AS_PARTNER_NAME,SO_VE_PARTNER,SO_VE_PARTNER_NAME,SO_AM_PARTNER,SO_AM_PARTNER_NAME,DL_WADAT_IST,SO_F_ZZ0S2MATUG,SO_F_AS_PARTNER,SO_F_AS_PARTNER_NAME,TM_DATEN,TM_DATBG,DL_ZZ0S2BLNR,SO_F_DGLTP,DL_CHARG,BL_VBELN_INV_LAST,BL_VBELN_INV_FIRST,TM_AR_DATE,DL_TRAID,SO_ZZ0S2REVG2,DL_VBELN,DL_POSNR,DL_LFIMG,DL_VRKME,DL_LFART,DL_LFART_VTEXT,TM_DPTEN,TM_DPTBG,SO_F_VBELN,SO_I_VBELN,TM_TDLNR,TM_TDLNR_NAME1,SO_GUSCON_LEVEL,SO_INCO1,SO_INCO2,SO_F_PSMNG,SO_F_AMEIN,SO_BASF_LOFCR,LAST_NOTE,SO_LEVEL_TYPE,SO_NETWR,SO_N_VBELN,BL_XBLNR,SO_ISCOMPLETED,TM_STTRG,TM_STTRG_DDTEXT,SO_ZTERM,DL_PEND_DEL_QUAN,DL_LFDAT,DL_WADAT,SO_F_WERKS,SO_BSARK,SO_BSARK_VTEXT,SO_F_VKORG,SO_F_VKORG_VTEXT,SO_KBETR,SO_PRSDT,SO_F_AUFNR,SO_BSTKD,SO_ROUTE,SO_VKGRP,SO_VKGRP_BEZEI,SO_ZZDKPPRODB,TM_TKNUM,TM_SHIPMENT_ALERT,TM_SHIPMENT_CURRENT_STATUS,SO_F_VSBED,SO_F_VSBED_VTEXT,TM_VSART,TM_VSART_BEZEI,SO_TRAGR,SO_TRAGR_VTEXT,SO_F_TDDAT,TM_EXTI1,DL_HSDAT,DL_VFDAT,SO_VBTYP,TM_TRACKING_ID_ELEM,TM_TRACKING_ID_COMP,SO_WAERK,SO_WAERS`)
      assert.equal(200, response.status)
    } catch (error) {
      assert.strictEqual(error.code, 'ECONNABORTED');
      assert.strictEqual(error.message, `timeout of ${timeout}ms exceeded`);
